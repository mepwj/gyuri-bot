...
- **출근/야근/퇴근**: 출근, 야근, 퇴근 응원 메시지와 퇴근 시간 안내

### 🎮 엔터테인먼트
- **규리 대화**: 봇 멘션 또는 `규리`가 포함된 메시지로 자연스럽게 대화
...
| 명령어 | 별칭 | 설명 | 사용법 |
|--------|------|------|--------|
| !안녕 | !hello, !hi | 규리가 인사해요! | !안녕 |
| !운세 | !ㅇㅅ, !fortune, !오늘운세, !오늘의운세 | 오늘의 운세, 운세 점수, 행운 아이템, 조언을 알려드려요. | !운세 [총운/애정운/금전운/건강운/직장운] |
| !별자리 | !ohaasa, !별자리운세, !일본운세, !오하아사 | 오하아사 오늘의 별자리 운세와 순위를 보여드려요. | !별자리 [별자리] |
| !출근 | !ㅊㄱ, !work | 출근 중인 분께 응원 메시지와 팁을 보내요. | !출근 또는 !ㅊㄱ |
| !야근 | !ㅇㄱ, !overtime | 야근 중인 분께 응원 메시지와 팁을 보내요. | !야근 또는 !ㅇㄱ |
| !퇴근 | !ㅌㄱ, !offwork, !gohome | 퇴근까지 남은 시간과 응원 메시지를 보내요. | !퇴근 또는 !ㅌㄱ |

### 대화 호출

| 호출 | 설명 | 사용법 |
|------|------|--------|
| @규리봇 | 봇을 멘션하면 AI 응답을 시도해요. 메시지가 없으면 기본 응답을 보내요. | @규리봇 [메시지] |
| 규리 포함 메시지 | 메시지에 `규리`가 포함되면 AI 응답을 시도해요. 최근 채널 대화 흐름도 함께 참고해요. | 규리 오늘 뭐해? |
